...
def computeEmbedding(n, d, S, alpha=1, num_random_restarts=0,max_num_passes=64,max_norm=0, epsilon=0.001, verbose=False):
    """
    ...
    while num_restarts < num_random_restarts:
        ...
        ts = time.time()
        X_new, emp_loss_new, hinge_loss_new, log_loss_new, acc_new = computeEmbeddingWithGD(X, S, alpha, max_iters=max_num_passes, max_norm=max_norm, epsilon=epsilon, verbose=verbose)
        te_gd = time.time()-ts
