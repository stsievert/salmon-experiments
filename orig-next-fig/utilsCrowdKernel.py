...
def computeEmbedding(n, d, S, mu=.05, num_random_restarts=0,max_num_passes=64,max_norm=0, epsilon=0.01, verbose=False):
    """
    ...
    while num_restarts < num_random_restarts:
        ...
        ts = time.time()
        X_new, emp_loss_new, log_loss_new, hinge_loss_new, acc_new = computeEmbeddingWithGD(X, S, mu, max_iters=max_num_passes, max_norm=max_norm, epsilon=epsilon, verbose=verbose)
        te_gd = time.time()-ts
...
def computeEmbeddingWithEpochSGD(n,d,S,mu=.05, max_num_passes=0,max_norm=0,epsilon=0.01,a0=0.1,verbose=False):
    """
...
def computeEmbeddingWithGD(X, S, mu=.05, max_iters=0, max_norm=0, epsilon=0.01, c1=0.0001, rho=.7, verbose=False):
    """
    ...
    return X,emp_loss_0,hinge_loss_0,log_loss_0
