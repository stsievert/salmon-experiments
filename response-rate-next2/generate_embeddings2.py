...
SAVE_DIR = Path("/scratch/ssievert/salmon-final-next2/out/")
...
def _check_version():
    ...
    assert "v0.8.1rc5" in salmon.__version__, (salmon.__version__, salmon.__file__)
    return True
...
if __name__ == "__main__":
    ...
    X_salmons = {
        ...
        for f in Path("salmon/io/2021-11-08/").glob("*.csv")
    }
    assert len(X_salmons) == 3
    ...
    for name, job in [
        ("salmon-arr", salmon_arr),
    ]:
        print(f"len({name}_job) =", len(job))
    job_kwargs = salmon_arr
